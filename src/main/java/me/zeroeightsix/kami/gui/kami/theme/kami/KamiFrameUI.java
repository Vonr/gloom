...
public class KamiFrameUI<T extends Frame> extends AbstractComponentUI<Frame>
{
    ...
    public KamiFrameUI() {
        ...
        this.centerY = false;
        this.xLineOffset = 0;
        this.cFontRendererLarge = new CFontRenderer(new Font("Verdana", 0, 24), true, false);
    }
    ...
    private void checkSettingGuiColour(final Setting setting) {
        ...
        switch (name2) {
            case "Border Red": {
                this.redForBG = (float) setting.getValue();
                break;
            }
            case "Border Green": {
                this.greenForBG = (float) setting.getValue();
                break;
            }
            case "Border Blue": {
                this.blueForBG = (float) setting.getValue();
                break;
            }
        }
    }
    ...
    public void renderComponent(final Frame component, final FontRenderer fontRenderer) {
        ...
        GL11.glEnable(3042);
        GL11.glEnable(3553);
        this.cFontRendererLarge.drawString(component.getTitle(), (float)(component.getWidth() / 2 - this.cFontRendererLarge.getStringWidth(component.getTitle()) / 2), 1.0f, Color.white.getRGB());
        GL11.glEnable(2884);
        GL11.glDisable(3042);
        GL11.glDisable(3553);
        int top_y = 5;
